...
\begin{document}
	...
	\begin{enumerate}
		\item \textbf{Prima componente}  : Separa i piloti che hanno un'alta variabilità (molte correzioni) nell'erogazione della potenza da quelli che hanno un'alta variabilità nella fase di decelerazione. A valori elevati corrisponde a uno stile di guida caratterizzato da una trazione più "sporca" o reattiva, con continue correzioni sul pedale dell'acceleratore che si contrappone uno stile caratterizzato da una frenata molto modulata e dinamica, mantenendo però un'uscita di curva molto composta e lineare nell'erogazione del gas.
		
		\item \textbf{Seconda componente} : distungue tra piloti che lavorano di più in termini di velocità pura (avanti/dietro) contro chi ha maggiore di percorrenza (destra/sinistra). A valorio elevati corrisponde uno stile di guida a "V", dove il pilota punta tutto sulla frenata profonda e sulla ripartenza rapida, variando molto l'accelerazione longitudinale. Invece, per valori bassi contrappone uno stile di guida basato sulla velocità di percorrenza, che predilige la gestione del carico laterale.
		
		\item \textbf{Terza componente} : questa componente entra nel dettaglio delle fasi della curva. Discrimina piloti che prediligono un'ingresso più aggressivo rispetto all'uscita di curva e viceversa. In particolare, a valori alti corrisponde uno stile di guida caratterizzato da una gestione dinamica e correttiva della fase di uscita. Mentre a valori bassi corrisponde uno stile caratterizzato da un inserimento in curva molto incisivo e variabile, dove il pilota lavora molto sui transitori di carico iniziali per forzare l'auto in corda.
		
		\item \textbf{Quarta componente} :  questa componente descrive fasi di guida mista ad alta energia cinetica. A valori elevati corrisponde una maggiore variabilità complessiva in manovre che combinano velocità sostenuta e forze laterali, mentre valori bassi indicano una guida più regolare e uniforme.
	...
	Il valore della distanza di  Kullback-Leibler $(KL)$ del modello risulta essere pari a $ 89.622$. Ciò dà sostegno a quanto già osservato prima graficamente, ulteriormente supportato dall'incertezza pari a $0.0796$ che esclude una possibile situazione di sovrapposizione dei gruppi.
