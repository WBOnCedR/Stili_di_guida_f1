...
pp <- function(data,GP=1,pilot=1){
  ...
  for (i in GP){
    ...
    for (k in pilot){
    S_no_l[[as.character(i)]][[as.character(k)]] <- S_new %>% filter(GP == i & Driver== k) %>% select(-GP,-Driver,-X,-Y,-Z,-Distance,-GP_Name)
...
piste <- function(data,GP=1){
  ...
  Ss <- list()
  for (i in GP){
    S_no_l[[as.character(i)]] <- S_new %>% filter(GP == i) %>% select(-GP,-Driver,-X,-Y,-Z,-Distance,-GP_Name)
    ...
    plot(clust,what="classification",main=paste("Classificazione per GP",i))
...
pil <- function(data,pilot=1){
  ...
  Ss <- list()
    for (k in pilot){
    S_no_l[[as.character(k)]] <- S_new %>% filter(Driver== k) %>% select(-GP,-Driver,-X,-Y,-Z,-Distance,-GP_Name)
    Ss[[as.character(k)]] <- S_new %>% filter(Driver== k) %>% select(-GP,-Driver)
    library(mclust)
    clust <- Mclust(S_no_l[[as.character(k)]])
    S_no_l[[as.character(k)]]$cluss <- clust$classification
    cat(paste("Modello pilota",k,clust$modelName,"con BIC pari a",clust$bic,"e",clust$G,"gruppi"),"\n")}
  for(k in pilot){
    plot(Ss[[as.character(k)]]$X,
         Ss[[as.character(k)]]$Y,
         col=S_no_l[[as.character(k)]]$cluss,main=paste("Pilota",k,"In GP",i)
         )}
