...
Eliminazione dei dati non coerenti con regole della *FIA* ovvero che se si è oltre il 107% del tempo minimo si esclude il pilota.
```{r}
tel.guida_summary %>% group_by(GP) %>% filter(laptime > 1.07*min(laptime))
...
```
Comp 1 : intensità del pilota su accelerazione/frenata longitudinale se bassa 
...
ggplot(tel.comp.labels, aes(x = Comp.3, y = Comp.4)) +
  # 1. Usa 'color' invece di 'fill' per i punti standard
  geom_point(aes(color = as.factor(class)), size = 3) + 
  
  # 2. Correggi il nome della colonna per le etichette (presumo sia 'pilota')
  # check_overlap = TRUE evita che le scritte si sovrappongano troppo
  geom_text(aes(label = fill_pilota), vjust = -1, size = 3, check_overlap = FALSE) + 
  
  # 3. Usa una scala di colori ad alto contrasto
  scale_color_brewer(palette = "Set1") +
  
  theme_minimal() +
  labs(title = "Plot Comp.3 vs Comp.4",
       color = "Classe") # Ora questo corrisponde all'aes(color)

ggplot(tel.comp.labels %>% filter(class==3), aes(x = Comp.3, y = Comp.4)) +
  # 1. Usa 'color' invece di 'fill' per i punti standard
  geom_point(aes(color = as.factor(class)), size = 3) + 
