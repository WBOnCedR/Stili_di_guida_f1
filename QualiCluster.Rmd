...
Visualizzazione delle correlazioni tra le variabili eliminando le variabili spaziali e temporali

```{r}
tel.num <- tel_resc %>% select(where(is.numeric)) %>% select(-time,-x,-y,-z,-distance,-rel_distance)
corr <- cor(tel.num)
ggcorrplot(corr,hc.order = T, type="lower", lab=T)
```
Le correlazioni confermano le ipotesi che l'accelerazione è utile al fine di capire lo stile gi guida in quanto fortemente correlata con brake e throttle inlotre c'è anche una correlazione forte tra speed ed gear, risulta più informativa la variabile speed quindi si sceglie di eliminare alcune variabili come gear throttle e brake 

```{r}
tel.guida <- tel_resc %>%
  
  select(GP, pilota,speed,rpm,time, acc_x, acc_y) %>% 
...
Una prima analisi delle componenti principali per comprendere quali variabili influiscono più sulla varianza
```{r}
tel_num <- tel.num %>% 
  
  ungroup() %>% 
  
  select(rpm,speed,drs,acc_x,acc_y,acc_z)
