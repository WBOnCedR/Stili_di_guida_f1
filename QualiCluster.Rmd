...
```{r}
tel.comp["class"] <- clust$classification
```
...
tel.comp$GP <- tel.pca$GP
tel.comp$pilota <- tel.pca$pilota
tel2 <- tel %>% group_by(GP,pilota) %>% summarize(laptime=max(lap_time),life=max(life),strategy=(max(strategy)),gomma=(max(gomma))) %>% ungroup()
tel.comp$laptime <- tel2$laptime
tel.comp$life <- tel2$life
tel.comp$strategy <- tel2$strategy
tel.comp$gomma <- tel2$gomma
