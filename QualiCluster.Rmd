                ...
                group_by(GP,pilota) %>% 
                  mutate(
                   ...
                   acc_x = if_else(acc_x > 0, acc_x, 0)) %>% 
                    mutate(acc_x=rescale(acc_x,to=c(0,1)),dec_x=rescale(dec_x,to=c(0,1),acc_y= abs(rescale(acc_y,to = c(0, 1))))) %>%                     ungroup()
...
tel2 <- tel %>% 
          group_by(GP,pilota) %>% 
              summarize(laptime=max(lap_time),life=max(life),strategy=(max(strategy)),gomma=(max(gomma)),.groups = "drop") %>% 
                  group_by(GP) %>% 
                      mutate(laptime=scale(as.numeric(laptime))) %>% ungroup()

tel.guida_summary$laptime <- tel2$laptime
...
tel.comp <- as_tibble(PCA$scores[,c(1:10)])
```
...
plot(tel.comp.labels$Comp.2,tel.comp$Comp.4,col=tel.comp.labels$class,pch=16)
plot(tel.comp.labels$Comp.2,tel.comp$Comp.7,col=tel.comp.labels$class,pch=16)
```
...
tel.data <- tel.comp[,c(2,4,7)]
tel.comp.labels$class <- as.factor(tel.comp.labels$class)
tel.class <- unlist( tel.comp.labels[,11] )
str(tel.class)
str(tel.data)

set.seed(543)
n <- nrow(tel.data)
test.set <- sample(1:n,20)
