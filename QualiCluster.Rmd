...
tel %>%
  group_by(GP) %>%
  filter(lap_time > 1.07 * min(lap_time))
```
...
for (gp in unique_GP) {
  p <- ggplot(tel.comp.labels %>% filter(GP == gp), aes(x = Comp.2, y = Comp.1)) +
    ...
    theme_minimal() +
    labs(
      title = paste0("Plot Comp.2 vs Comp.1 ", gp),
      fill = "Classe"
    )
  print(p)
}
...
## Regressione con Mixture of Experts

Per ridurre l'effetto della configurazione della pista (quarta componente) e concentrarsi sullo stile di guida, si applica un modello di regressione con mixture of experts utilizzando `stepFlexmix`. Questo approccio permette di condizionare i parametri della distribuzione alla variabile TRACK (Comp.4).

```{r}
# Rinomina delle componenti per maggiore chiarezza
