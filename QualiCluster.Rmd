...
-   `Comp.1` : Dinamica longitudinale complessiva e variabilità dell'utilizzo del gas e del freno.
    -   **Score elevati**: indicano una dinamica longitudinale più con maggiori variazioni di accelerazione e frenata e una modulazione più marcata del gas e del freno. Possono riflettere uno stile di guida più aggressivo.
...
library(fmsb)
for (i in 1:5){
tel.load <- PCA$loadings[,i] 
tel.load <- abs(tel.load)
tel.load <- sort(tel.load, decreasing = TRUE)
tel.load <- tel.load[1:10]

df_load <- as.data.frame(t(tel.load))

max <- max(df_load) * 1.2 
min <- 0

df_final <- rbind(rep(max, 10), rep(min, 10), df_load)
...
radarchart(df_final,
           ...
           title = paste("Profilo Componente Principale",i,"(Top 10 Loadings)"),
           vlabels = c(ifelse(str_detect(colnames(df_final),"acc_x"),str_replace(colnames(df_final),"acc_x","acc_long"),
                              ifelse(str_detect(colnames(df_final),"acc_y"),str_replace(colnames(df_final),"acc_y","acc_lat"),
                                     ifelse(str_detect(colnames(df_final),"dec_x"),str_replace(colnames(df_final),"dec_x","dec"),
                                            ifelse(str_detect(colnames(df_final),"throttle"),str_replace(colnames(df_final),"throttle","acceleratore"),
                                                   ifelse(str_detect(colnames(df_final),"brake"),str_replace(colnames(df_final),"brake","freno"),
                                                          ifelse(str_detect(colnames(df_final),"speed"),str_replace(colnames(df_final),"speed","velocità"),0))))))))
}
...
set.seed(789)
vec <- data.frame(CV=NA,BIC=NA,MODEL=NA,TOT_MISS=NA,MISS=NA,N=NA)
n_test <- list()

   for (m in listmod){
     for (i in 1:1000){
test.set <- sample(1:nrow(tel.data),40)
res <-  mixmodLearn(tel.data[-test.set,], tel.class[-test.set], 
                  models=mixmodGaussianModel(listModels=m,equal.proportions=F),criterion= c("CV","BIC"))
...
pred <-  mixmodPredict(tel.data[test.set,], classificationRule=res@bestResult)
  ...
  cat(cat(paste("Progress",m,"\n")))
  vec$MODEL <- rep(m,1000)
  n_test[[as.character(m)]]   <- vec 
}
