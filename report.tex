...
\begin{document}
	...
	Per l'analisi la variabile di accelerazione laterale, viene trasformata tramite valore assoluto, così da ottenere una misura dell'intensità complessiva (magnitudo)\footnote{Estratto codice R consultabile nell'appendice \ref{trattamento variabili}.\label{appendice_3}}
	
	Per l'accelerazione longitudinale invece vengono create due nuove variabili\textsuperscript{\ref{appendice_3}}.\\
	
	Accelerazione, che assimila tutti i valori positivi della variabile originaria,Decelerazione che al contrario della precedente assimila, tutti i valori negativi della variabile originaria, successivamente viene applicato il valore assoluto.
	...
	Ogni statistica descrittiva viene impiegata per creare il coefficiente di variazione, con struttura del tipo:
	\[\dfrac{sd(x)}{mean(x)}\]
	Al fine di eliminare l'effetto di scala.
	
	\clearpage
	
	
	Un ultimo accorgimento riguarda la variabile tempo giro, in quanto per regolamento della \textbf{FIA}(federazione internazionale dell'automobile) un pilota se supera il $107\%$ del tempo giro minore viene escluso dalle qualifiche.
	
	Applicando questo filtro al dataset, emergono come outlier i piloti Hamilton, Tsunoda, Antonelli, Albon e Bortoleto nel GP di Las Vegas e Stroll nel GP d'Olanda.
	
	Analizzando il contesto, si nota che a Las Vegas i tempi più elevati sono imputabili alle condizioni meteo avverse (pioggia intensa), seguite da un progressivo asciugamento della pista che ha avvantaggiato gli altri piloti. Nel caso di Stroll, invece, il dato si riferisce a un giro di lancio completo, poiché nel momento del giro di qualifica non è stato completato il giro per un incidente.
	
	Di conseguenza per eliminare il rumore, si elimina soltanto l'occorrenza di Stroll, in quanto il giro non rappresenta il giro "migliore" ma solamente il riscaldamento.\\
	 
		
	Il nuovo dataset pertanto contiene 30 variabili, che forniscono informazione sullo stile di guida del pilota nella specifica gara.
	
	Il dato è pertanto, ancora influenzato dal tracciato di ogni gran premio, per ovviare, si riscalano tutte le variabili nell'intervallo $[0,1]$
	...
	In particolare la funzione analizza le coppie di variabili con correlazione eccessiva e rimuove quella che, in media, risulta più correlata con tutte le altre. Questo processo di scrematura permette di ridurre la dimensionalità a 21 variabili.
	
	\subsubsection{Analisi delle componenti principali}
	Seppur la dimensionalità sia diminuita, 21 dimensioni sono eccessive sia ai fini di interpretabilità che per sforzo computazionale per un model base clustering.
	
	
	Si passa dunque da un analisi delle componenti principali (PCA) ai fini di ridurre la dimensionalità.
	Successivamente l'applicazione della PCA \footnote{Estratto del codice R consultabile nell'appendice \ref{PCA}}, dall'analisi risulta che le prime 6 componenti spiegano l'$80\%$ della varianza, come si nota nella figura \ref{fig:pca}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=.5\textwidth]{tel_pca.pdf}
		\caption{Varianza Spiegata Comulata}
		\label{fig:pca}
	\end{figure}
