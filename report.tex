...
\begin{document}
	...
	\begin{center}
		\begin{tabular}{llll}
			...
			Tempo dal via          & $\mathrm{s}$        & $[0, +\infty)$                    & \texttt{numeric} \\
			Distanza percorsa      & $\mathrm{m}$        & $[0, +\infty)$                    & \texttt{numeric} \\
			Distanza relativa      & -- 				 & $[0, 1]$                          & \texttt{numeric} \\
			Velocità               & $\mathrm{km/h}$     & $[0, 400]$                        & \texttt{numeric} \\
			Regime motore          & RPM                 & $[0, 15000]$                      & \texttt{numeric} \\
			Marcia                 & --                  & $\{1, \dots, 8\}$                 & \texttt{numeric} \\
			Freno                  & --                  & $\{0, 1\}$                        & \texttt{factor} \\
			Acceleratore           & $\%$                & $[0, 100]$                        & \texttt{numeric} \\
			DRS                    & --                  & $\{0, 1\}$                        & \texttt{factor} \\
			Accelerazione laterale e longitudinale      & $\mathrm{g}$        & $[-6, +6]$                        & \texttt{numeric} \\
